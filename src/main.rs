...
use crate::{spinner::{Spinner, SpinnerResult}, code_generation::CodeGenerator};
use clap::Parser;
        ...
        while !sender.is_closed() {
            ...
            let _: JoinHandle<Result<(), reqwest::Error>> = tokio::spawn(async move {
                ...
                match resp.status().as_u16() {
                    ...
                    302 => { // Invalid image
                        if loop_sender.send(StatusReport::Invalid(url.to_string())).is_err() {
                            return Ok(());
                        }
                    },
                    200 | 206 => { // Valid image
                        if loop_sender.send(StatusReport::Valid(url.to_string())).is_err() {
                            return Ok(());
                        }
                    },
                    _ => {}
                    ...
                    if args.spinner {
                        match spinner.next()  {
                            SpinnerResult::TooSoon(ch) => {
                                if args.tries {
                                    print!("\r{} Attempt {} ({})", ch, &attempt, &url);
                                    stdout().flush().unwrap();
                                }
                            }
                            SpinnerResult::Ok(ch) => {
